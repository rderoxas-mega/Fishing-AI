...
if uploaded_file:
    ...
    df = df.sort_values('datetime_full') # Sort for time-series features like lags
    ...
    df = df.drop(columns=['UID', 'date', 'drivetime'], errors='ignore')
    ...
    # Lag features - rely on QTYTUBS. If QTYTUBS is not present, lags will be NaN.
    # For prediction, if QTYTUBS exists, use it. If not, consider a strategy for default/imputation.
    # Assuming QTYTUBS is present in the uploaded data for lag calculation.
    if 'QTYTUBS' in df.columns:
        df['lag_1'] = df['QTYTUBS'].shift(1)
        df['lag_2'] = df['QTYTUBS'].shift(2)
    else:
        # If QTYTUBS is not available, fill lags with a placeholder or handle appropriately
        df['lag_1'] = 0 # Placeholder
        df['lag_2'] = 0 # Placeholder

    # Drop rows with NaN values introduced by lagging or other operations
    df = df.dropna()
